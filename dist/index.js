...
import { CredentialSession, Agent, BlobRef } from '@atproto/api';
...
var BlobRefGeneratorSchema = z8.object({
  $type: z8.literal("blob-ref-generator"),
  ref: z8.object({
    $link: z8.string()
  }),
  mimeType: z8.string(),
...
var FileGeneratorSchema = z8.object({
  name: z8.string(),
  type: z8.string(),
  dataBase64: z8.string()
});
...
var loginFactory = (allowedPDSDomainSchema) => {
  return publicProcedure.input(
    z8.object({
      handlePrefix: z8.string().regex(/^[a-zA-Z0-9-]+$/),
...
var StrongRefSchema = z8.object({
  $type: z8.literal("com.atproto.repo.strongRef").optional(),
  uri: z8.string().regex(/^at:\/\//),
  cid: z8.string()
});
...
var ClaimActivityInputSchema = z8.object({
  title: z8.string().min(1, "Title is required"),
  shortDescription: z8.string().min(1, "Short description is required"),
  description: z8.string().optional(),
  locations: StrongRefSchema.array().min(1, "At least one location is required"),
  project: z8.string().optional(),
  workScopes: z8.array(z8.string()).min(1, "At least one work scope is required"),
  startDate: z8.string(),
  endDate: z8.string(),
  contributors: z8.array(z8.string()).min(1, "At least one contributor is required"),
  createdAt: z8.string().optional()
});
var ClaimActivityUploadsSchema = z8.object({
...
var VisibilitySchema = z8.enum(["Public", "Hidden"]);
var OrganizationInfoInputSchema = z8.object({
  displayName: z8.string().min(1, "Display name is required"),
  shortDescription: z8.string().min(1, "Short description is required"),
  longDescription: z8.string().min(1, "Long description is required"),
  website: z8.string().url("Website must be a valid URL").optional(),
  logo: BlobRefGeneratorSchema.optional(),
  coverImage: BlobRefGeneratorSchema.optional(),
  objectives: z8.array(ObjectivesSchema).min(1, "At least one objective is required"),
  startDate: z8.string().datetime().optional(),
  country: z8.string().min(1, "Country is required"),
  visibility: VisibilitySchema,
  createdAt: z8.string().datetime().optional()
});
var OrganizationInfoUploadsSchema = z8.object({
...
var LayerInputSchema = z8.object({
  name: z8.string().min(1, "Layer name is required"),
  type: LayerTypeSchema,
  uri: z8.string().url("Layer URI must be a valid URL"),
  description: z8.string().optional(),
  createdAt: z8.string().datetime().optional()
});
...
var LocationInputSchema = z8.object({
  name: z8.string().min(1, "Location name is required")
});
var LocationUploadsSchema = z8.object({
  shapefile: z8.union([z8.string().url(), FileGeneratorSchema])
});
...
var LocationUpdateInputSchema = z8.object({
  name: z8.string().min(1, "Location name is required"),
  shapefile: z8.object({
    $type: z8.literal("app.gainforest.common.defs#smallBlob"),
    blob: BlobRefGeneratorSchema
  }).optional()
});
var LocationUpdateUploadsSchema = z8.object({
...
var AppRouterFactory = class {
  constructor(_allowedPDSDomains) {
    ...
    this.allowedPDSDomainSchema = z8.enum(this.allowedPDSDomains);
...
export { GainforestSDK, createContext, supportedPDSDomainSchema };
